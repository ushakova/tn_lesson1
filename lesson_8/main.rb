...
def main_menu
  ...
  puts '14. Посмотреть список всех вагонов поезда'
  puts '15. Выход'
  choise = gets.to_i
  sub_menu(choise)
end
...
def sub_menu(choise)
  ...
  when 9 then take_place_in_wagon
  when 10 then move_forward
  when 11 then move_back
  when 12 then print_stations_list
  when 13 then print_trains_list_on_station
  when 14 then print_wagons_list
  when 15 then return
  end
  main_menu
end
...
def delete_station_from_route
  return if @route.nil? || @route.stations.size == 2
...
def attach_wagon_to_train
  train = chose_train
  return unless train
  train.type == PassengerTrain ? puts('Число мест:') : puts('Объём:')
  number = gets.to_i
  wagon = train.type == PassengerTrain ? PassengerWagon.new(number) : CargoWagon.new(number)
  train.attach(wagon)
end
...
def chose_wagon
  ...
  puts 'Выберите вагон:'
  choise = gets.to_i
  choise <= @train.wagons.size ? @train&.wagons[choise - 1] : chose_wagon
end
